...
////////////////////////////// Get data from OPC DA   1 ///////////////////////////////////

func getDataFromOPCda_1(tags []string, itemOPC string, ipOPC string, portOPC string, db_name string, db_ip string, db_port string, measurement string) []float64{
	...
	//println(strSend)
		...
		for i:=0; i < len(tags); i++ {
		data:= match_1(string(replyTags), string(tags[i]))
		...
		write_influxdb_1(measurement, tags, resultData, db_name, db_ip, db_port)
...
func match_1(reply string, tagName string) string {
...
//////////////////////////////  Write data to influx 1 ///////////////////////////////////



func write_influxdb_1(measurement string, tag_name []string, data []float64, db_name string, db_ip string, db_port string) string{
...
func readJson_1() {
	...
	for true {
		...
		for i := 1; i < number_of_measurements + 1; i++ {
			...
			getDataFromOPCda_1(tag_names, itemOPC, ipOPc, portOPC, db_name, db_ip, db_port, measurement)
...
////////////////////////////// Get data from OPC DA   2 ///////////////////////////////////

func getDataFromOPCda_2(tags []string, itemOPC string, ipOPC string, portOPC string, db_name string, db_ip string, db_port string, measurement string) []float64{
	...
	strSend := `{"id":2,"operation":"read","items":{` + strTags +`}}`
		...
		//println("tags length:: ", len(tags))
		for i:=0; i < len(tags); i++ {
		data:= match_2(string(replyTags), string(tags[i]))
		...
		write_influxdb_2(measurement, tags, resultData, db_name, db_ip, db_port)
...
func match_2(reply string, tagName string) string {
	i := strings.Index(reply, tagName)
	//println("index :  " , i)
	if i >= 0 {
		j := strings.Index(reply[i+len(tagName)+3:], `"`)
...
//////////////////////////////  Write data to influx 2 ///////////////////////////////////



func write_influxdb_2(measurement string, tag_name []string, data []float64, db_name string, db_ip string, db_port string) string{
...
////////////////////////////// Function to read json 2///////////////////////////////////


func readJson_2() {

	// Open our jsonFile
	jsonFile, err := os.Open("C_config_main_2.json")
	// if we os.Open returns an error then handle it
	if err != nil {
		fmt.Println(err)
		time.Sleep(3 * time.Second)
	...
	fmt.Println("Successfully Opened C_config_main_2.json")
	// defer the closing of our jsonFile so that we can parse it later on
	defer jsonFile.Close()
	...
	for true {
		...
		for i := 1; i < number_of_measurements + 1; i++ {
			...
			measurement := tag_group.Measurement
			getDataFromOPCda_2(tag_names, itemOPC, ipOPc, portOPC, db_name, db_ip, db_port, measurement)
